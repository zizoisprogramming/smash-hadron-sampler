...
int generate()
{
 ...
 // ---< end thermal densities calc
  ...
  while(cumulantDensity[isort]<xsort) isort++ ;
   ...
   if(strcmp(params::coordinateSystemHydro, "milne")==0) {
   ...
   else if(strcmp(params::coordinateSystemHydro, "cartesian")==0) {
    ...
    throw std::invalid_argument(std::string("Only 'Milne' or 'Cartesian' are "
      "supported as coordinate systems for the hydro evolution. Provided was '")
      + std::string(params::coordinateSystemHydro) +
